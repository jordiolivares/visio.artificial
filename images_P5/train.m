...
PSI = % #to-do: calculate the mean face of the training data
% #to-do: visualize PSI
A = % #to-do: calculate the matrix A by subtracting the mean face from each feature vector (help repmat)

% What is the trick of the method here?
S_trick = A'*A;

[V, D] = % #to-do: calculate the eigenvector and the eigenvalue of the trick matrix (help eig)
% Which is the dimension of V? Compare to the dimension of the original images. 

U = % #to-do: initialize eigenfaces taking into account the dimensionality reduction

% What is the following step here representing?
for l = 1: % #to-do: specifiy the limit of the loop
    ul = double(zeros(size(A,1),1));
    for k = 1: % #to-do: specifiy the limit of the loop   
        ul = ul + V(l,k)*A(:,k); 
    end
    U(:,l) = ul; 
end
    
dd = % #to-do: extract the eigen values of the matrix D (help diag)
[dd idx] = % #to-do: sort them in descendent order
